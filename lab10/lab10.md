...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   - `20`  
   ![](images/ws_3_1.png)  
   - `Total Length - Header Length = 56 - 20 = 36`  
   ![](images/ws_3_2.png)
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - `Identification` и `Header Checksum`. `Time to Live`, вообще говоря, не изменяется от каждой дейтаграммы к следующей, однако при переходе traceroute к следующему хопу он таки увеличивается на 1.  
      ![](images/ws_4_1_1.png)
      ![](images/ws_4_1_2.png)
      ![](images/ws_4_1_3.png)
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - `Version`, `Header Length`, `Total Length`, `Protocol`, `Source Address`, `Destination Address`, `Differentiated Services Field`, `Flags` и `Fragment Offset`
      - `Source Address` и `Destination Address` - ибо все пакеты идут от моего компьютера к одному и тому же адресу; `Protocol` - потому что в Linux traceroute отправляет именно UDP-пакеты; `Total Length` - так как размер пакета в этом запуске одинаковый; `Version` - ибо во всех рассматриваемых пакетах используется IPv4; `Header Length` - потому что IP-заголовок во всех этих пакетах имеет одинаковую структуру и не содержит дополнительных служебных полей; `Differentiated Services Field` - так как для всех этих traceroute-пакетов используется одинаковый режим обработки трафика; `Flags` - ибо в этом запуске маленькие пакеты не фрагментируются и никаких специальных флагов фрагментации не выставляется; `Fragment Offset` - аналогично, ведь тут никакие пакеты не являются фрагментами.
      - `Time to Live` - ибо traceroute целенаправленно должен увеличивать его; `Identification` - потому что у каждой новой IP-дейтаграммы должен быть свой идентификатор; `Header Checksum` - так как он пересчитывается заново при изменении IP-заголовка.
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Значение этого поля изменяется с каждым последующим исходящим пакетом (например, на скриншоте выше значения равны `0x9fec`, `0x07ad`, `0x2941`, `0x914f` и т.д.).
      - Явной простой закономерности я не вижу, однако если глянуть ICMP-пакеты от маршрутизатора к моему компьютеру, то там действительно есть закономерность в том, что каждое следующее значение Identification увеличивается на 1 *(при рассмотрении конкретного фиксированного маршрутизатора, т.е., например, если рассматривать лишь фреймы 330, 331 и 333)*. Однако это уже не исходящие пакеты моего компьютера, так что можно сказать, что закономерности всё же нет.  
      ![](images/ws_4_3.png)
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - `TTL` остаётся неизменным и равен `65`; `Identification` же неизменным НЕ остаётся, а, как и говорилось ранее, увеличивается на 1 с каждым новым пакетом.  
   ![](images/ws_6_1.png)
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Да. Конкретно у меня первая большая UDP-дейтаграмма была разбита на 3 IP-фрагмента: фреймы 99, 100 и 101. У всех этих трёх фрагментов одинаковое значение поля Identification, но разные значения Fragment Offset.  
      ![](images/ws_8_1_0.png)
      ![](images/ws_8_1_1.png)
      ![](images/ws_8_1_2.png)
      ![](images/ws_8_1_3.png)
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - `Total Length`, `Flags` *(конкретно флаг `More fragments`)*, `Fragment Offset` и `Header Checksum`
