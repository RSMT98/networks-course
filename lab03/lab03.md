...
сервер ожидает клиентов на определенном фиксированном порту. Также заранее создайте
фиксированное число рабочих потоков для обслуживания клиентов. При получении нового
TCP-соединения от клиента основной поток должен передать это соединение одному из
свободных рабочих потоков, а тот должен обработать запрос и отправить ответ. После
завершения обработки рабочий поток не уничтожается, а возвращается к ожиданию следующего
клиента. Иными словами, в этом пункте требуется переиспользование заранее созданных
потоков, а не создание нового потока на каждый запрос.
...
получает запрос от клиента, и при этом все рабочие потоки заняты, то запрос от клиента
блокируется (встает в очередь) и дожидается, пока не освободится один из потоков. После
этого запрос передается освободившемуся потоку на обработку. В этом пункте
`concurrency_level` задает размер пула рабочих потоков, которые должны быть созданы
заранее и переиспользоваться между запросами.
