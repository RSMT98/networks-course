...
LOG_PATH = Path("proxy_with_caching.log").resolve()
...
class ProxyWithCachingHandler(BaseHTTPRequestHandler):
        ...
        try:
            ...
            if (
                ...
                self._write_log(
                    ...
                    extra_msg="CACHE HIT | validated by 304 Not Modified",
                )
                return
            ...
            extra_caching_msg = None
            if self.command == "GET":
                if cache_status == "MISS":
                    extra_caching_msg = "CACHE MISS | stored on disk"
                elif cache_status == "UPDATED":
                    extra_caching_msg = "CACHE UPDATED | server returned a new version"
                elif cache_status == "REVALIDATE":
                    extra_caching_msg = "CACHE REVALIDATE | validators were sent, server returned full response"
                else:
                    extra_caching_msg = f"CACHE {cache_status}"

            self._write_log(target_url, resp.status, extra_msg=extra_caching_msg)
...
server = ThreadingHTTPServer((PROXY_HOST, args.port), ProxyWithCachingHandler)
print(f"Proxy server with caching is running and listening: {PROXY_HOST}:{args.port}")
try:
